...
public final class Recursion2 {
    ...
    public boolean splitOdd10(int[] nums) {
        ...
        int sum2 = 0;
        int i = 0;
        return sumParts2(nums, i, sum1, sum2);
    }

    public boolean sumParts2(int[] nums, int index, int sum1, int sum2) {
        if (index >= nums.length) {
            return sum1 % 10 == 0 && sum2 % 2 == 1;
        }
        return sumParts2(nums, index + 1, sum1 + nums[index], sum2)
                || sumParts2(nums, index + 1, sum1, sum2 + nums[index]);
    }
    ...
    public boolean split53(int[] nums) {
        ...
        int sum1 = 0;
        int sum2 = 0;
        return splitSum(nums, sum1, sum2, index);
    }

    public boolean splitSum(int[] nums, int sum1, int sum2, int index) {
        if (index >= nums.length) {
            if (sum1 == sum2) {
                return true;
            }
        ...
        if (nums[index] % 5 == 0) {
            return splitSum(nums, sum1 + nums[index], sum2, index + 1);
        }
        if (nums[index] % 3 == 0) {
            return splitSum(nums, sum1, sum2 + nums[index], index + 1);
        }
        return splitSum(nums, sum1 + nums[index], sum2, index + 1)
                || splitSum(nums, sum1, sum2 + nums[index], index + 1);
    }
