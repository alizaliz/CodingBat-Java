...
public final class Logic2 {
    ...
    public boolean isClose(int test, int comp) {
        return Math.abs(test - comp) <= 1;
    }

    public boolean isFar(int test, int compA, int compB) {
        return Math.abs(test - compA) >= 2 && Math.abs(test - compB) >= 2;
    }
    ...
    public int makeChocolate(int small, int big, int goal) {
        ...
        required = goal % 5;
        if (big - goal / 5 < 0) {
            required += 5 * Math.abs(big - goal / 5);
        }
        if (required > small)
            return -1;
        return required;
