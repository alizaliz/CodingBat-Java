...
public class Arrays2 {
    ...
    public int[] post4(int[] nums) {
        ...
        while (length > 0 && nums[length] != 4) {
            length--;
        }

        int[] ret = new int[nums.length - length - 1];

        for (int i = 0; i < ret.length; i++) {
            ret[i] = nums[length + i + 1];
        }
     ...
     * 3, 5, 5, 2] notAlone([3, 4], 3) → [3, 4] Go...S
     */
        ...
        for (int i = 0; i < nums.length; i++) {
            if (nums[i] == val && i > 0 && i < nums.length - 1) {
                if (nums[i - 1] != val && nums[i + 1] != val) {
                    ret[i] = nums[i - 1] > val && nums[i - 1] > nums[i + 1] ? nums[i - 1] : nums[i + 1];
                    continue;
                }
            }
            ret[i] = nums[i];
    ...
    public int largestOdd(int[] nums, int i) {
        int odd = 0;
